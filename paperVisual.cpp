...
//#define mergedAlign //the last step  align the merged cloud, dateset from maize 04132019 
//#define leafsegvisual //the last step align the sgementaed cloud, dateset from maize 04132019
#define corn2nd
//#define merge_all_pcz
...
inline void alignCloud(PointCloudT::Ptr cloud, PointCloudT::Ptr cloud_out, bool flip);
...
inline void translateCloud(PointCloudT::Ptr cloud, PointCloudT::Ptr cloud_out);
	...
	char* argv[])
{
	...
	std::vector<string> ids = {"WT_0ml_3"};
//"WT_0ml_3","WT_100ml_2","WT_500ml_1","WT_1000ml_5","WT_2000ml_3"
	std::vector<string> dates = {"06162018","06172018","06182018","06192018","06202018"};
	...
	for (auto &i:ids){
		...
		for (auto &j:dates){
			...
			string path = "/media/lietang/easystore/RoAD/cornValidation/corn2nd/"+j+"/leafSegResults/"+i+".pcd"; //leafSegResults

			if (pcl::io::loadPCDFile<PointT>(path, *cloud) == -1) //* load the file
			{

				PCL_ERROR("Couldn't read segmented file \n");
				cout << path<<endl;

				path = "/media/lietang/easystore/RoAD/cornValidation/corn2nd/"+j+"/merged/"+i+".pcd"; 

				if(pcl::io::loadPCDFile<PointT>(path, *cloud)==-1){

					PCL_ERROR("Couldn't read merged file \n");
					cout << path<<endl;
		...
		pcl::io::savePCDFile("/media/lietang/easystore/RoAD/cornValidation/corn2nd/cloud_all_days_"+i+".pcd", *final_cloud);
	...
	viewer.addPointCloud(final_cloud, to_string(cv::getTickCount()), v1);

	viewer.addPointCloud(final_cloud, to_string(cv::getTickCount()), v2);
	...
	std::vector<string> ids = {"WT_0ml_3","WT_100ml_2","WT_500ml_1","WT_1000ml_5","WT_2000ml_3"};
	...
	for (auto &i:ids){
		...
		string path = "/media/lietang/easystore/RoAD/cornValidation/corn2nd/cloud_all_days_"+i+".pcd"; //leafSegResults
		...
		pcl::io::savePCDFile("/media/lietang/easystore/RoAD/cornValidation/corn2nd/cloud_all_pczs.pcd", *final_cloud);
...
//if big plants, rotate point cloud to align the widest dimension along with x axis
	...
	//if(ratio>0.5f)
		alignCloud(cloud_in,cloud_in, 0);
...
inline void translateCloud(PointCloudT::Ptr cloud, PointCloudT::Ptr cloud_out){
//this function trasnlate the point cloud to the origin
	...
	*cloud_backup += *cloud;
	...
	cout<<"lowestP: "<<lowestP.x<<", "<<lowestP.y<<", "<<lowestP.z<<endl;
